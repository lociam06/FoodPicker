...
function EditFood(){
    ...
    useEffect(() => {
        ...
        const fetchIngredients = async () => {
            try {
                ...
                setFoodIngredientsList(result.map(x => ({
                    id: x.id,
                    quantity: x.quantity,
                    unitId: x.unitId,
    ...
    useEffect(() => {
        setFoodNameValue(food.name);
        setFoodImages(food.images || []);
        setDecriptionValue(food.decription);
        setEatTimeValue(food.eat_time);
        setMinPriceValue(food.min_price);
        setMaxPriceValue(food.max_price);
        setRateValue(food.rate);
        setHealthyValue(food.healthy);
        setDificultyValue(food.difficulty);
        setDuractionValue(food.approximate_preparation_time);
        setProcedureList(food.recipe?.procedure.split(" -|- ").map(x => ({
            id: 0,
            procedure: x,
            foodId: 0
        })) || []);
    ...
    //States para el form
    const [ foodNameValue, setFoodNameValue ] = useState(food.name);
    const [ foodImages, setFoodImages ] = useState<FoodImage[]>(food.images || []);
    const [ descriptionValue, setDecriptionValue ] = useState(food.decription);
    const [ eatTimeValue, setEatTimeValue] = useState(food.eat_time);
    const [ minPriceValue, setMinPriceValue ] = useState(food.max_price);
    const [ maxPriceValue, setMaxPriceValue ] = useState(food.max_price);
    const [ rateValue, setRateValue] = useState(food.rate);
    const [ healthyValue, setHealthyValue] = useState(food.healthy);
    const [ dificultyValue, setDificultyValue] = useState(food.difficulty);
    const [ durationValue, setDuractionValue ] = useState(food.approximate_preparation_time);
    ...
    const handleEditFood = async () => {
        ...
        const result = await FoodService.UpdateFood(foodToAdd, recipeToAdd, foodIngredientsList, foodImages);

        if(result){
            await Swal.fire({
                title: "Se ha actualizado la comida correctamente",
                icon: "success",
                confirmButtonText: "Aceptar",
            });
            ...
            Swal.fire({
                title: "Ha ocurrido un error al actulizar la comida",
                icon: "error",
            ...
            <main>
                ...
                <div className="food-content box-shadow-normal">
                    ...
                    <div className="info">
                        ...
                        <div>
                            ...
                            <EatTimeSelect selected={eatTimeValue} onChange={selectEatTimeValue}/>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <RateSelect selected={rateValue} onChange={selectRateValue}/>
                        </div>
                        <div>
                            <span className="font-bold">Saludable: </span>
                            <HealthySelect selected={healthyValue} onChange={selectHealthyValue}/>
                        </div>
                        <div>
                            <span className="font-bold">Dificultad: </span>
                            <DificultySelect selecte={dificultyValue} onChange={selectDificultyValue}/>
                        </div>
                        <div>
                ...
                <div className="recipe-container">
                                    ...
                                    <li key={i}>
                                        <div className='ingredient box-shadow-normal'>
                                            <div>
                                                ...
                                                <IngredientsSelect selected={ing.ingredientId} onChange={(value) => {
                                            ...
                                            <div>
                                                ...
                                                <UnitSelect selected={ing.unitId} onChange={(value) => changeUnit(value, i)}/>
                                            </div>
                                            <div>
                                                <span className='font-bold'>Es obligatorio</span>
                                                <input checked={!ing.is_optional} type="checkbox" onChange={(e) => changeIsOptional(e.target.checked, i)}/>
                                            </div>
                    ...
                    <div className='action-btn flex justifie-end gap-1'>
                        <button className='btn btn-primary' onClick={handleEditFood}>Guardar</button>
